...
/*
===============================================================================
Address expression parsing rules (CUSTOM ASM SYNTAX for AmmAsm2)
===============================================================================
We intentionally restrict the addressing syntax to avoid Intel's chaos
and make parsing predictable. This is NOT NASM/MASM syntax — it's OUR rule btw.

Allowed form:
    [BASE/Physical-Address + REG/DISP * SCALE]

Where:
    BASE  - one 64-bit register (e.g., rax, rbx, rcx, r8, r15)
    DISP  - integer displacement (in bytes, mandatory if SCALE present)
    SCALE - 1, 2, 4, or 8 (default: 1 if omitted)

Examples:
    [rax + 20]         ; base + displacement (allowed)
    [r8 + 4 * 8]       ; base + displacement * scale (allowed)
    [rbx + rcx * 2]    ; base + displacement * scale (hex displacement) (allowed)
    [0xB8000]          ; physical address (for writing kernel becouse AmmAsm can gen 16bit code) (allowed)
    [rbx * 4]          ; base + scale (allowed)

Not allowed:
    [SCALE * DISP + BASE]   ; wrong order!
    [DISP + BASE]           ; displacement first!
    [BASE + SCALE]          ; scale without displacement!
    [rax]                   ; bare base register without displacement!

 Why this gospel exists:
 - Simple to parse, no NASM schizophrenia
 - Avoids guessing games like "is that a number or a reg?"
 - Prevents the parser from having PTSD
 - Fits in my brain without Intel's 200-page addressing bible

 Implementation note:
 Parser should:
   1) Read BASE (register)
   2) Expect '+'
   3) Read DISP (number, decimal or hex)
   4) If '*' is found, read SCALE and validate (1, 2, 4, 8)
===============================================================================
*/
...
AddrExpr parse_addr_expr(const char *expr, int line) {
    ...
    char *mull = strchr(buf, '*'); 
    if (plus) *plus = 0;

    // base
    if (is_reg(buf) == 4 || is_reg(buf) == 5) {
        a.base_is_reg = 1;
        strncpy(a.base_reg, buf, sizeof(a.base_reg));
    } 
    else {
        // adress
        a.base_val = eval_expr(buf);
    }

    // index/disp*scale
    if (plus) {
        char *part = plus + 1;
        char *star = strchr(part, '*');
        if (star) {
            *star = 0;
            if (is_reg(part) == 4 || is_reg(part) == 5) {
                a.index_is_reg = 1;
                strncpy(a.index_reg, part, sizeof(a.index_reg));
            } 
            else a.index_val = eval_expr(part);
            a.scale = eval_expr(star + 1);
            if(a.scale != 1 && a.scale != 2 && a.scale != 4 && a.scale != 8){
                fprintf(stderr, "AmmAsm: unvalid adress scale at line '%d' \nIntel only supports *1, *2, *4, *8\n", line);
                exit(1);
            }
        } 
        else {
            if (is_reg(part) == 4 || is_reg(part) == 5) {
                a.index_is_reg = 1;
                strcpy(a.index_reg, part);
                a.scale = 1;
            } 
            else {
                a.index_val = eval_expr(part);
                a.scale = 1;
            }
        }
    } 
    // waiting this struct -> "rbx*2"
    else if(!plus && mull) {
        *mull++ = '\0';
        a.scale = eval_expr(mull);
        if (is_reg(mull) != 0) {
            fprintf(stderr, "AmmAsm: at line '%d' you used register as scale, thats unvalid \nUse instead use 1,2,4,8\n", line);
            exit(1);
        }
    }
