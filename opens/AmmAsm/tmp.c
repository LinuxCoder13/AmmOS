...
const char *HUMAN_AST[] = {"BYTE", "BYTE PTR", "QWORD", "QWORD PTR", "DWORD", "DWORD PTR", "LBYTE", "LBYTE PTR", 
# 61 "Aasm.c" 3 4
                                                                                                                ((void *)0)
# 61 "Aasm.c"
                                                                                                                    };
const char *HUMAN_TOKEN[] = {"inst", "label", "reg", "[reg]", "[reg +-*/ value]", "string", 
# 62 "Aasm.c" 3 4
                                                                                           ((void *)0)
# 62 "Aasm.c"
                                                                                               };
...
const char *regs8[] = {"al", "ah", "bl", "bh", "cl", "ch", "dl", "dh", 
# 73 "Aasm.c" 3 4
                                                                      ((void *)0)
# 73 "Aasm.c"
                                                                          };
const char* regs16[] = {"ax", "bx", "cx", "dx", "si", "di", "sp", "bp", 
# 74 "Aasm.c" 3 4
                                                                       ((void *)0)
# 74 "Aasm.c"
                                                                           };
const char* regs32[] = {"eax", "ebx", "ecx", "edx", "esi", "edi", "esp", "ebp", 
# 75 "Aasm.c" 3 4
                                                                               ((void *)0)
# 75 "Aasm.c"
                                                                                   };
...
# 209 "Aasm.c" 3 4
               stderr
# 209 "Aasm.c"
                     , "AmmAsm:\033[;31m Fatal: Too mant tokens (max 1024)\033[0m\n");
        exit(1);
    }
...
# 278 "Aasm.c" 3 4
                                 stderr
# 278 "Aasm.c"
                                       , "AmmAsm: invalid escape sequence on line %d\n", line); exit(1);
            }
        }
        else {
            *dest++ = *buff++;
...
# 351 "Aasm.c" 3 4
               stderr
# 351 "Aasm.c"
                     , "AmmAsm:\033[;31m Fatal: no such file or dir\033[0m\n");
        exit(1);
    }
...
# 452 "Aasm.c" 3 4
                           stderr
# 452 "Aasm.c"
                                 , "AmmAsm: unterminated string literal at line %d\n", line);
                    exit(1);
                }
...
# 472 "Aasm.c" 3 4
                                   stderr
# 472 "Aasm.c"
                                         , "AmmAsm: Unknown escape sequence on line %d\n", line);
                            exit(1);
                    }
                    buff += 2;
                }
...
# 482 "Aasm.c" 3 4
                           stderr
# 482 "Aasm.c"
                                 , "AmmAsm: Unterminated character literal on line %d\n", line);
                    exit(1);
                }
                         buff++;
                char tmp2[2] = {tmp, '\0'};
...
# 572 "Aasm.c"
            ...
            else if (strncmp(buff, "resb", 4) == 0){ add_token(0x19, buff, line); buff += 4; while (
# 591 "Aasm.c" 3 4
                                                                                                     ((*__ctype_b_loc ())[(int) ((
# 591 "Aasm.c"
                                                                                                     *buff
...
# 591 "Aasm.c"
            ...
            else if (strncmp(buff, "resq", 4) == 0){ add_token(0x1a, buff, line); buff += 4; while (
# 592 "Aasm.c" 3 4
                                                                                                     ((*__ctype_b_loc ())[(int) ((
# 592 "Aasm.c"
                                                                                                     *buff
...
# 592 "Aasm.c"
            ...
            else if (strncmp(buff, "resd", 4) == 0){ add_token(0x1b, buff, line); buff += 4; while (
# 593 "Aasm.c" 3 4
                                                                                                     ((*__ctype_b_loc ())[(int) ((
# 593 "Aasm.c"
                                                                                                     *buff
...
# 593 "Aasm.c"
            ...
            else if (strncmp(buff, "resl", 4) == 0){ add_token(0x1c, buff, line); buff += 4; while (
# 594 "Aasm.c" 3 4
                                                                                                     ((*__ctype_b_loc ())[(int) ((
# 594 "Aasm.c"
                                                                                                     *buff
...
# 596 "Aasm.c" 3 4
                         stderr
# 596 "Aasm.c"
                               , "AmmAsm: unvalid syntax or char on line", line); exit(1); }
            buff++;
        }
    ...
    while(toks[pos].type != 0x0d){
        ...
        if(tok->type == 0x0f && (pos == 0 || toks[pos-1].type == 0x0a)){
            ...
            while(pos < toks_count){
                ...
                if (node.bytes.size >= 256) { fprintf(
# 677 "Aasm.c" 3 4
                                                     stderr
# 677 "Aasm.c"
                                                           , "AmmAsm: Too many bytes in string literal at line %d\n", toks[pos].line); exit(1); }
                if(toks[pos].type == 0x17){
                    ...
                    while (*s) {
                    if (node.bytes.size >= sizeof(node.bytes.data)){ fprintf(
# 691 "Aasm.c" 3 4
                                                                            stderr
# 691 "Aasm.c"
                                                                                  , "AmmAsm: String too long in BYTE directive at line %d\n", toks[pos].line); exit(1);}
...
# 698 "Aasm.c" 3 4
                           stderr
# 698 "Aasm.c"
                                 , "AmmAsm: Bytes are too cooked at line %d\n", toks[pos].line);
                    exit(1);
                }
...
# 718 "Aasm.c" 3 4
                               stderr
# 718 "Aasm.c"
                                     , "AmmAsm: too many BYTEPTRS (max. 256)\n");
                        exit(1);
                    }
...
# 736 "Aasm.c" 3 4
                           stderr
# 736 "Aasm.c"
                                 , "AmmAsm: expected label name (T_ID) or 0 (NULL) in BYTEPTRS at line %d\n", toks[pos].line);
                    exit(1);
                }
            }
            ...
            while(pos < toks_count){
                if (toks[pos].type == 0x0c || toks[pos].type == 0x0d) break;
                if(toks[pos].type == 0x18){ pos++; continue;}
                if(node.qwords.size >= 256) fprintf(
# 752 "Aasm.c" 3 4
                                                   stderr
# 752 "Aasm.c"
                                                         , "AmmAsm: to many qword leterals at line \"%d\"", toks[pos].line);
...
# 760 "Aasm.c" 3 4
                           stderr
# 760 "Aasm.c"
                                 , "AmmAsm: unexpected token in QWORD at line %d: '%s'\n", toks[pos].line, toks[pos].value);
                    exit(1);
                }
            }
            ast[ast_count++] = node;
            ...
            while (pos < toks_count){
                if(toks[pos].type == 0x0c || toks[pos].type == 0x0d)break;
                if(toks[pos].type == 0x18){ ++pos; continue; }
                if(node.qwordptrs.size >= 256){ fprintf(
# 776 "Aasm.c" 3 4
                                                       stderr
# 776 "Aasm.c"
                                                             , "AmmAsm: Too many qwordptrs at line \"%d\"", toks[pos].line); exit(1);}
...
# 789 "Aasm.c" 3 4
                           stderr
# 789 "Aasm.c"
                                 , "AmmAsm: expected label name (T_ID) or 0 (NULL) in QWORDPTRS at line %d\n", toks[pos].line);
                    exit(1);
                }
            }
            ast[ast_count++] = node;
            ...
            while (pos < toks_count && toks[pos].type != 0x0c && toks[pos].type != 0x0d) {
                if (toks[pos].type == 0x18){ pos++; continue; }
                if (node.dwords.size >= 256){ fprintf(
# 804 "Aasm.c" 3 4
                                                     stderr
# 804 "Aasm.c"
                                                           , "AmmAsm: Too many dwords at line %d\n", toks[pos].line); exit(1);}
                if (toks[pos].type == 0x01) {
...
# 811 "Aasm.c" 3 4
                           stderr
# 811 "Aasm.c"
                                 , "AmmAsm: unexpected token in DWORD at line %d: '%s'\n", toks[pos].line, toks[pos].value);
                    exit(1);
                }
            }
            ...
            while (pos < toks_count){
                if(toks[pos].type == 0x0c || toks[pos].type == 0x0d)break;
                if(toks[pos].type == 0x18){ ++pos; continue; }
                if(node.dwordptrs.size >= 256){ fprintf(
# 827 "Aasm.c" 3 4
                                                       stderr
# 827 "Aasm.c"
                                                             , "AmmAsm: Too many dwordptrs at line \"%d\"", toks[pos].line); exit(1);}
...
# 840 "Aasm.c" 3 4
                           stderr
# 840 "Aasm.c"
                                 , "AmmAsm: expected label name (T_ID) or 0 (NULL) in DWORDPTRS at line %d\n", toks[pos].line);
                    exit(1);
                }
            }
            ast[ast_count++] = node;
            ...
            while (pos < toks_count && toks[pos].type != 0x0c && toks[pos].type != 0x0d) {
                if (toks[pos].type == 0x18){ pos++; continue; }
                if (node.lbytes.size >= 256){ fprintf(
# 855 "Aasm.c" 3 4
                                                     stderr
# 855 "Aasm.c"
                                                           , "AmmAsm: Too many lbytes at line %d\n", toks[pos].line); exit(1);}
                if (toks[pos].type == 0x01) {
...
# 862 "Aasm.c" 3 4
                           stderr
# 862 "Aasm.c"
                                 , "AmmAsm: unexpected token in LBYTE at line %d: '%s'\n", toks[pos].line, toks[pos].value);
                    exit(1);
                }
            }
            ...
            while (pos < toks_count){
                if(toks[pos].type == 0x0c || toks[pos].type == 0x0d)break;
                if(toks[pos].type == 0x18){ ++pos; continue; }
                if(node.lbyteptrs.size >= 256){ fprintf(
# 878 "Aasm.c" 3 4
                                                       stderr
# 878 "Aasm.c"
                                                             , "AmmAsm: Too many lbyteptrs at line \"%d\"", toks[pos].line); exit(1);}
...
# 891 "Aasm.c" 3 4
                           stderr
# 891 "Aasm.c"
                                 , "AmmAsm: expected label name (T_ID) or 0 (NULL) in LBYTEPTRS at line %d\n", toks[pos].line);
                    exit(1);
                }
            }
            ast[ast_count++] = node;
...
# 903 "Aasm.c" 3 4
                       stderr
# 903 "Aasm.c"
                             , "AmmAsm: syntax erorr. expected identifier before \"%s\" and decimal number after\n",
                    (pp->type == 0x19) ? "resb" :
                    (pp->type == 0x1a) ? "resq" :
                    (pp->type == 0x1b) ? "resd" :
                    (pp->type == 0x1c) ? "resl" : "?");
